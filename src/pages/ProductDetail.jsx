...
import useUIStore from "../store/useUIStore"; // Adjust path/casing if your filename is different
...
export default function ProductDetail() {
  ...
  const openEnquiryModal = useUIStore((state) => state.openEnquiryModal);

  const product = data.find(
    (item) => item.itemname.toLowerCase().replace(/\s+/g, "-") === slug,
  );
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-10">
      <button
        onClick={() => navigate(-1)}
        className="flex items-center text-sm text-gray-400 hover:text-accent mb-8 transition-colors"
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
        ...
        <div>
          ...
          <h1 className="font-serif text-3xl md:text-5xl font-bold mb-8">
            {product.itemname}
          </h1>
          ...
          <div className="border-t border-border">
            {product.itemprops.map((prop, idx) => (
              <div
                key={idx}
                className="flex justify-between py-4 border-b border-border text-sm"
              >
          ...
          <div className="mt-10 flex gap-4">
            ...
            <button
              ...
              className="bg-accent text-black px-6 py-3 rounded-lg font-semibold text-sm hover:bg-[#e8c98e] transition-colors"
            >
              Enquire Now
            </button>
          </div>
